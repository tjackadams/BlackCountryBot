Param(
    ...
    [parameter(Mandatory = $false)][string]$appName = "eblackcountry",
    ...
    [parameter(Mandatory = $false)][string]$tlsSecretName = "eblackcountry-tls-custom",
...
function Install-Chart {
    ...
    if ($customRegistry) {
        $options = "$options --set inf.registry.server=$registry --set inf.registry.login=$dockerUser --set inf.registry.pwd=$dockerPassword --set inf.registry.secretName=eblackcountry-docker-scret"
    }
    
    if ($chart -ne "eblackcountry-common" -or $customRegistry) {
        # eblackcountry-common is ignored when no secret must be deployed        
        $command = "install $appName-$chart $chart $options"
...
if ($sslSupport -eq "staging") {
    $sslEnabled = $true
    $tlsSecretName = "eblackcountry-letsencrypt-staging"
    $sslIssuer = "letsencrypt-staging"
}
elseif ($sslSupport -eq "prod") {
    $sslEnabled = $true
    $tlsSecretName = "eblackcountry-letsencrypt-prod"
...
if ($clean) {
    ...
    helm delete  $(helm ls -q --filter 'eblackcountry') 
...
Write-Host "Begin eBlackCountry installation using Helm" -ForegroundColor Green
...
if ($deployInfrastructure) {
    ...
    Write-Host "eBlackCountry infrastructure (bbdd, redis, ...) charts aren't installed (-deployCharts is false)" -ForegroundColor Yellow
}
...
if ($deployCharts) {
    ...
    Write-Host "eBlackCountry non-infrastructure charts aren't installed (-deployCharts is false)" -ForegroundColor Yellow
}
